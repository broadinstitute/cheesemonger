...
Works with the separate-screen model where each screen is an independent
store under a dataset root directory.
"""
...
from cheesemonger.simulate import CHUNK_PRESETS, SCALE_PRESETS
...
def build_query_specs(
    ...
    fixed_2 = {"timepoint": timepoint, "testedperturbation": perturbation}
    fixed_1 = {"timepoint": timepoint}

    return [
        # --- Series queries (fix timepoint+perturbation, read all gene expressions) ---
        QuerySpec(name="series/ZScore", datatype="ZScore", constraints=fixed_2),
        QuerySpec(name="series/neg_log10_FDR", datatype="neg_log10_FDR", constraints=fixed_2),
        QuerySpec(name="series/L2FC", datatype="L2FC", constraints=fixed_2),
        QuerySpec(name="series/TestMean", datatype="TestMean", constraints=fixed_2),
        QuerySpec(name="series/CtrlMean", datatype="CtrlMean", constraints=fixed_2),
        QuerySpec(name="series/nTestCells", datatype="nTestCells", constraints=fixed_2),
        ...
        # Diagonal query disabled — simulated data uses different label
        # prefixes ("Gene_" vs "RGene_") so the diagonal is always empty.
        # Uncomment when running against real data with matching labels.
        #
        # QuerySpec(
...
def run_benchmark(
    ...
    specs = build_query_specs(timepoint, perturbation)
    results: list[BenchmarkResult] = []

    for spec in specs:
        logger.info("Running: %s (%d warmup + %d measured)", spec.name, warmup, n_iterations)
        result = run_one_query(
            dataset_root, fmt, screen, spec, perturbations, n_iterations, warmup,
        )
...
def print_results(results: list[BenchmarkResult], chunk_preset: str = "big") -> None:
    ...
    chunk_desc = f"(1, {preset['testedperturbation']}, {preset['testedgeneexpression']})"
    chunk_mb = 1 * preset["testedperturbation"] * preset["testedgeneexpression"] * 4 / (1024 ** 2)
...
def run_concurrent_benchmark(
    ...
    all_specs = build_query_specs(timepoint, perturbation)
    ...
    for n_threads in concurrency_levels:
        ...
        _timed_query(dataset_root, fmt, screen, work[0])
        ...
        with ThreadPoolExecutor(max_workers=n_threads) as pool:
            futures = [
                pool.submit(_timed_query, dataset_root, fmt, screen, spec)
                for spec in work
            ]
...
def main():
    ...
    parser.add_argument(
        ...
        default="small",
        help="Scale preset (determines dataset name for path construction).",
    )
    parser.add_argument(
        "--chunk-preset",
    ...
    schema = pesca_schema(**(SCALE_PRESETS[args.scale]))
    ...
    concurrency_levels = args.concurrent if args.concurrent else [1, 2, 4, 8]

    if run_sequential:
        all_results: list[BenchmarkResult] = []
        for fmt in formats_to_test:
            dataset_root = args.data_dir / f"chunks_{args.chunk_preset}" / dataset_name / fmt
        ...
        for fmt in formats_to_test:
            ...
            logger.info("Concurrent benchmark: %s at %s", fmt.upper(), dataset_root)
