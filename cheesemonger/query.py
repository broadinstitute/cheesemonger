...
def list_screens(dataset_root: Path) -> list[str]:
    """Return the list of active screen labels from the registry."""
    reg_path = dataset_root / "_registry.json"
    if reg_path.exists():
...
def open_screen(
    ...
    """
    Open a single screen's store and return an xr.Dataset.

    For Zarr the dataset is lazily backed by dask arrays.
    For NetCDF pass chunks={} to get lazy dask-backed loading as well.
    """
    if fmt == "zarr":
        store_path = dataset_root / screen / "data.zarr"
        return xr.open_zarr(str(store_path))
    elif fmt == "netcdf":
        nc_path = dataset_root / screen / "data.nc"
        return xr.open_dataset(str(nc_path), chunks={})
    else:
...
def get_vector(
    ...
    ----------
    dataset_root : Path
        Root directory of the dataset (contains screen subdirectories).
    fmt : str
        Storage format — "zarr" or "netcdf".
    screen : str
        Screen label (determines which store to open).
    datatype : str
        Name of the datatype shard to read (e.g. "ZScore", "L2FC").
    constraints : dict
        Dimension values to fix, e.g. {"timepoint": 4, "testedperturbation": "MYC"}.
